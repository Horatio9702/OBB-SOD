...
from clip_text import class_names_clip, class_names_imagenet, class_names, new_class_names, BACKGROUND_CATEGORY#, imagenet_templates
import argparse
...
def perform(process_id, dataset_list, args, model, bg_text_features, fg_text_features, cam):
    ...
    for im_idx, im in enumerate(tqdm(databin)):
        ...
        xmlfile = img_path.replace('/image', '/Annotations')
        ...
        for obj in data["object"]:
            obj["name"] = class_names_clip[class_names_clip.index(obj["name"])]
            if obj["name"] not in label_list:
                label_list.append(obj["name"])
                label_id_list.append(class_names_clip.index(obj["name"]))
        ...
        for image in ms_imgs:
            ...
            for idx, label in enumerate(label_list):
                keys.append(class_names_clip.index(label))
                ...
                box, cnt = scoremap2bbox(scoremap=grayscale_cam, threshold=0.6, multi_contour_eval=True)
                ...
                cam_refined = torch.from_numpy(cam_refined_highres)
                cam_refined = cam_refined.unsqueeze(0)
                cam_refined_avg = torch.nn.AvgPool2d(kernel_size=3, stride=1, padding=1)(cam_refined)
                cam_refined_min = -torch.nn.MaxPool2d(kernel_size=3, stride=1, padding=1)(-cam_refined)
                cam_refined = torch.where(cam_refined < cam_refined_avg, (cam_refined + cam_refined_min) / 2.0, cam_refined)
                cam_refined = torch.squeeze(cam_refined)
                cam_refined_highres = cam_refined.cpu().numpy().astype(np.float32)
        ...
        rcc = refined_cam_all_scales.cpu().numpy().astype(np.float16)
        rcc = (np.squeeze(rcc) * 255).astype(np.uint8)
        img = Image.fromarray(rcc, mode='L')
        img.save(os.path.join(args.cam_out, im.replace('jpg', 'png')))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--img_root', type=str, default='RGB image path')
    parser.add_argument('--split_file', type=str, default='train.txt')
    parser.add_argument('--cam_out_dir', type=str, default='cam path')
    parser.add_argument('--cam_out', type=str, default='cam png path')
